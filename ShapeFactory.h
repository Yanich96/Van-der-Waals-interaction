...
std::vector<Oscillator> create_cube(int atomsInEdge, real distance) {
    std::vector<Oscillator> system_atoms;
    for (int x = 0; x < atomsInEdge; ++x) {
        for (int y = 0; y < atomsInEdge; ++y) {
            for (int z = 0; z < atomsInEdge; ++z) {
                system_atoms.emplace_back(x * distance, y * distance, z * distance);
            }
        }
    }
    return system_atoms;
...
std::vector<Oscillator> create_parallepiped(int atomsInWidth, int atomsInHeight, int atomsInDepth, real distance) {
    std::vector<Oscillator> system_atoms;
    for (int x = 0; x < atomsInWidth; ++x) {
        for (int y = 0; y < atomsInHeight; ++y) {
            for (int z = 0; z < atomsInDepth; ++z) {
                system_atoms.push_back({x * distance, y * distance, z * distance});
            }
        }
    }
    return system_atoms;
