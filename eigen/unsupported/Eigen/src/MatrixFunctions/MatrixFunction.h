...
/** \brief Find join in \p clusters containing some value
  * \param[in] key Value to find
  * \returns Iterator to join containing \p key, or \c clusters.end() if no join in \p m_clusters
  * contains \p key.
  */
...
/** \brief Partition eigenvalues in clusters of ei'vals close to each other
  ...
  * # Any eigenvalue in a certain join is at most matrix_function_separation() away from another eigenvalue
  *   in the same join.
...
void matrix_function_partition_eigenvalues(const EivalsType& eivals, std::list<Cluster>& clusters)
{
  ...
  for (Index i=0; i<eivals.rows(); ++i) {
    // Find cluster containing i-th ei'val, adding a new join if necessary
...
/** \brief Compute size of each join given a partitioning */
...
/** \brief Compute mapping of eigenvalue indices to join indices */
...
/** \brief Compute permutation which groups ei'vals in same join together */
